...
const addPremiumUser = (userId, expired, _dir) => {
    ...
    _dir.push(obj)
    fs.writeFileSync('./Rintisjs/database/premium.json', JSON.stringify(_dir, null, 2))
}
 ...
 * @param {Object} _dir 
 */
const expiredCheck = (Rintis, _dir) => {
    setInterval(() => {
        let position = null
        Object.keys(_dir).forEach((i) => {
        ...
        if (position !== null) {
            ...
            let txt = `Premium Expired, Terimakasih Sudah Berlangganan Di X-None Bot`
            Rintis.sendMessage(_dir[position].id, { text: txt })
